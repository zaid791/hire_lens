...
\subsection{SLA}
\begin{itemize}[leftmargin=*]
  \item \textbf{Availability SLA}: 99.5\% monthly availability for bot command handling and report retrieval.
\end{itemize}

\subsection{SLOs}
\begin{itemize}[leftmargin=*]
  \item \textbf{Latency SLO}: 95\% of bot commands respond within 2 seconds when using cached or lightweight data.
  \item \textbf{Analysis SLO}: 90\% of profile analyses complete within 60 seconds for standard GitHub profiles.
  \item \textbf{Reliability SLO}: 99\% of scheduled notifications execute within 5 minutes of the planned time.
\end{itemize}

\subsection{SLIs}
\begin{itemize}[leftmargin=*]
  \item Telegram command success rate.
  \item API latency percentiles (p50, p95, p99).
  \item Profile analysis completion time.
  \item Notification job execution delay.
  \item OAuth login success rate and active session count.
\end{itemize}
